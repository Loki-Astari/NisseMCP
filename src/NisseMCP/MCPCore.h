...
class MCPCore
{
    ...
    MCPCoreConfig   config;

    protected:
        MCPCoreConfig const& getConfig() const {return config;}

    public:
        MCPCore(MCPCoreConfig const& config);

    protected:
        // Decode InputSream
        bool handleInputStream(Context& context);
...
ThorsAnvil_MakeTrait(ThorsAnvil::Nisse::MCP::MCPCoreConfig);
